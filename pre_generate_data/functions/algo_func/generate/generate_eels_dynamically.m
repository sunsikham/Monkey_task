function [curr_trial_data, game_opt] = generate_eels_dynamically(curr_trial_data, visual_opt, game_opt)
    ...
    % Determine if eels will swap sides during the trial
    swap_sides = rand() < game_opt.swap_eels_prob;
    ...
    for iE = 1:game_opt.n_eels
        ...
        eels(iE).final_side = swap_sides * (3 - sides(iE)) + ~swap_sides * sides(iE);
        ...
        while ~valid_candidate
            if eels(iE).final_side == 1
                % Left eel: angle in [5pi/6, 7pi/6] i.e. 150° to 210°
                angle = (5*pi/6) + rand() * ((7*pi/6) - (5*pi/6));
            else
                % Right eel: angle in [-pi/6, pi/6] i.e. -30° to 30°
                angle = (-pi/6) + rand() * ((pi/6) - (-pi/6));
            end
