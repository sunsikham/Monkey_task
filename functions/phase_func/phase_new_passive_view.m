function [data, left_eel_final_pos, right_eel_final_pos, ...
          left_fish_final_pos, right_fish_final_pos] = ...
         phase_new_passive_view(data, visual_opt, game_opt, ...
    ...
    num_steps = round(visual_opt.refresh_rate *5) + randi([30, 60]);
    frame_duration = 1 / visual_opt.refresh_rate;
    frame_threshold = frame_duration - game_opt.buffer_t;
    
    % ──────────── Decide which eel(s) to move ────────────
    if strcmp(phase_str, 'PV')
        move_left_eel = true;
        move_right_eel = true;
    elseif strcmp(phase_str, 'PV1')
        move_left_eel = rand() > 0.5;
        data.pv1_side_left = move_left_eel;
        move_right_eel = ~move_left_eel;
    ...
    % Generate initial positions for both eels
    ...
    for t_step = 1:num_steps
        ...
        if move_right_eel
            ...
            draw_eel(right_eel_fut_pos, right_eel_color, 150, ...
    ...
    left_eel_final_pos  = left_eel_curr_pos;   % <─ 리턴용
    right_eel_final_pos = right_eel_curr_pos; 
    left_fish_final_pos  = left_fish_curr_pos;   % ← 추가
    right_fish_final_pos = right_fish_curr_pos;  % ← 추가
