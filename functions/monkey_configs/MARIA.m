function config = MARIA()
    ...
    config.competency_switch_offset = 3; % First competency switch at trial 3
    config.min_comp_switch = 15;      % Minimum trials between competency switches
    config.max_comp_switch = 20;      % Maximum trials between competency switches
    
    
    config.competency_noise_level = 0.02;

    % Device settings
    config.device.KEYBOARD = true;
    config.device.JOYSTICK = false;
    ...
    % Visual settings
    config.visual.screen_number = 0; % Primary screen
    ...
    config.game.competencies = [0.01, 0.9]; % Competency values for eels
    
    config.game.n_eels = 2;                % Number of eels in the game
    config.game.eel_sz = 30;               % Size of eels (pixels)
    config.game.eel_spd = 1;               % Speed ofp the eel (pixels/second)
