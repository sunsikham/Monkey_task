function config = WILLY()
    ...
    config.n_trials = 400;            % Total number of trials
    
    config.reliability_offset = 2;    % First reliability switch at trial 2
    config.min_rel_switch = 23;       % Minimum trials between reliability switches
    config.max_rel_switch = 27;       % Maximum trials between reliability switches
    
    config.competency_switch_offset = 10; % First competency switch at trial 3
    config.min_comp_switch = 25;      % Minimum trials between competency switches
    config.max_comp_switch = 30;      % Maximum trials between competency switches
    
    
    config.competency_noise_level = 0;
    ...
    % Visual settings
    config.visual.screen_number = 1; % Primary screen
    ...
    config.visual.alpha = 0.3; % Transparency (0 to 1)
    ...
    config.game.pursuit_time = 10;          % Duration of pursuit phase (seconds)
    ...
    % Reward Parameters
    config.game.reward_duration = 0.3;       % Duration of reward (seconds)
    config.game.short_reward_duration = 0.09; % Duration of short reward (seconds)
    config.game.reward_value = 1; % For humans
    
    % Eel Configuration
    config.game.electrical_field = 270;    % Electrical field around the eel
    config.game.competencies = [0.05, 0.8]; % Competency values for eels
    ...
    config.game.fast_spd = 17;             % Speed of the fish when moving quickly (pixels/second)
    config.game.R = 250;                    % Larger radius around eel (pixels) where fish can move in pursuit
