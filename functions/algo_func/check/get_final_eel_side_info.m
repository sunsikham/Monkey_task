function [left_eel_curr_pos, right_eel_curr_pos, ...
          ...
          get_final_eel_side_info(curr_trial_data, game_opt)
    % 이 함수는 오직 'final_side'만을 기준으로 왼쪽/오른쪽을 명확히 구분합니다.
    % trial의 최종 결과를 요약할 때 사용합니다.

    % [최종 수정] 어느 인덱스(1 또는 2)가 왼쪽(side 1)에 해당하는지 명시적으로 찾습니다.
    if curr_trial_data.eels(1).final_side == 1
        left_idx = 1;
        right_idx = 2;
    else % eels(2).final_side가 1일 수밖에 없으므로
        left_idx = 2;
        right_idx = 1;
    end
    
    % 이제 찾아낸 인덱스를 사용하여 모든 변수에 값을 정확하게 할당합니다.
    % 이렇게 하면 'left_eel_color'에는 왼쪽에 있던 eel의 색상이,
    % 'right_eel_color'에는 오른쪽에 있던 eel의 색상이 정확히 담기게 됩니다.
